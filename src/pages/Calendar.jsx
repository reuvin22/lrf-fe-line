...
function Calendar() {
  ...
  const { transportationExpenses, setTransportationExpenses} = useTransportationExpensesContext()
  ...
  const handleClick = async (day) => {
    ...
    // Filter attendance for the selected day
    const filtered = calendar.filter(
      (item) => formatDateFromApi(item.work_date) === formatted
    );

    setAttendanceCalendar(filtered);

    try {
      // Fetch all transportation expenses in parallel
      const allExpensesResponses = await Promise.all(
        filtered.map((att) =>
          transportationExpensesApi
            .getAll({ attendance_id: att.id })
            .then((res) => res?.data?.data || [])
            .catch((err) => {
              console.error("Error fetching transport for attendance:", att.id, err);
              return [];
            })
        )
      );
      ...
      const allExpenses = allExpensesResponses.flat();

      setTransportationExpenses(allExpenses);
    } catch (err) {
      console.error("Error fetching transportation expenses:", err);
      setTransportationExpenses([]); // fallback to empty array
    }
  ...
  const getStatusForDay = (day) => {
    ...
    const found = calendar.find(
      (item) => formatDateFromApi(item.work_date) === formattedDate
    );
    ...
    if ((found.status === "WORKING" || found.status === "END_OF_DAY") && hasSegments) {
      return "done";
    }
  ...
  const renderStatus = (day) => {
    ...
    if (status === "done") return <span className="w-1.5 h-1.5 bg-green-500 rounded-full mt-1"></span>;
    if (status === "missing") return <span className="w-1.5 h-1.5 bg-orange-400 rounded-full mt-1"></span>;
